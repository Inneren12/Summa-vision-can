...
Non-debt context that future contributors should be aware of but that
doesn't require tracked resolution.

### 2026-04-20 — Editor token tuning for WCAG AA

Stage 4 Task 5 (contrast matrix validator) discovered that two
canvas-rendered design tokens failed WCAG AA normal-text contrast
(4.5:1) on the canonical `#0B0D11` base background:

- `TK.c.txtM` was `#5C6370` → 3.22:1 (used by `eyebrow_tag` and
  `source_footer` text).
- `pal.neg` (all palettes) was `#E11D48` → 4.14:1 (used by
  `delta_badge` when `direction === "negative"`).

Both were tuned inline during the Task 5 rollout to pass the new
validator:

- `TK.c.txtM`: `#5C6370` → `#7A818C` (≈ 4.96:1). Design hierarchy
  `txtM < txtS < txtP` preserved.
- `pal.neg`: `#E11D48` → `#F43F5E` (≈ 5.60:1) across all six
  palettes (`housing`, `government`, `energy`, `society`, `economy`,
  `neutral`). Same red semantic, slightly brighter hue.

Future design refreshes should be aware these tokens were chosen to
pass the validator, not freely from the brand palette. `TK.c.err`
(editor-chrome error state, not canvas-rendered) retains the original
`#E11D48`.
