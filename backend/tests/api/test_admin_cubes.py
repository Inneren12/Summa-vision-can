...
def _set_test_api_key(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # We must patch the specific AuthMiddleware instance that was already
    # instantiated in main.py, as it captures the setting value at import time.
...
async def test_sync_requires_auth(
    ...
    resp = await client.post("/api/v1/admin/cubes/sync")
    assert resp.status_code == 401
...
async def test_get_cube_requires_auth(
    ...
    resp = await client.get("/api/v1/admin/cubes/14-10-0127-01")
    assert resp.status_code == 401
