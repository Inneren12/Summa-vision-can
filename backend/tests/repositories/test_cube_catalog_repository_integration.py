...
async def test_search_english_fts(pg_session: AsyncSession) -> None:
    ...
    await repo.upsert_batch(
        [
            _cube("14-10-0001-01", "Vacancy rates for rental apartments in Canada"),
            _cube("18-10-0001-01", "Consumer Price Index monthly"),
        ]
    )
...
async def test_search_typo_tolerance(pg_session: AsyncSession) -> None:
    ...
    await repo.upsert_batch(
        [
            _cube("14-10-0003-01", "Vacancy rates for rental apartments in Canada"),
            _cube("18-10-0003-01", "Consumer Price Index monthly"),
            _cube("36-10-0003-01", "International trade statistics"),
        ]
    )
    await pg_session.commit()
...
async def test_upsert_batch_returns_deterministic_count(
    ...
    repo = CubeCatalogRepository(pg_session)
    cubes = [_cube(f"99-10-{i:04d}-01", f"Test cube {i}") for i in range(10)]
