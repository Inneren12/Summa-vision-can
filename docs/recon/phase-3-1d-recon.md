...
**period=null semantics (locked, P2 + Codex follow-up):** when `period` is null, snapshot means "latest series value as observed in `semantic_value_cache` at capture time." On compare, ResolveService is called with `period=None` through the cached-only resolve path (per §3.2 BLOCKER-1) and returns the latest series value as currently present in `semantic_value_cache`. Compare is bounded by cache freshness — it does NOT pull from StatCan and does NOT auto-prime. Snapshot's captured `value_at_publish` is compared against the current cached latest. Drift in cached latest-period values surfaces as `value_changed` reason. If the cube has not been re-primed since capture, the compare may return `fresh` even if upstream StatCan has new data; cache refresh is the responsibility of the priming workflow (e.g. APScheduler `priming_jobs`), not compare. If the cache row is missing entirely, comparator emits `compare_failed` with `resolve_error="RESOLVE_CACHE_MISS"`. Future PR may add an alternative "frozen-period" binding mode that resolves a stable historical period instead of "latest."
