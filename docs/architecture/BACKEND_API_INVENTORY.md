...
| POST | `/api/v1/admin/publications/{publication_id}/publish` | `backend/src/api/routers/admin_publications.py:545` | `_get_repo`, `_get_audit`, `_get_staleness_service` (Phase 3.1d) | `PublicationPublishRequest \| None` (`backend/src/schemas/staleness.py:173`) — optional body added in Phase 3.1d for `bound_blocks` snapshot capture; backward compatible (no body / null / `{}` all work; bare-array body → 422) | 200 → `PublicationResponse`; 404 → `PublicationNotFoundError` | Phase 3.1d — when `bound_blocks` is present, invokes `PublicationStalenessService.capture_for_publication` with default `allow_auto_prime=True` (recon §5.3). Capture is best-effort: per-block resolve / shape-validation failures are logged inside the service and never raise into the caller, so publish success is never rolled back. |
