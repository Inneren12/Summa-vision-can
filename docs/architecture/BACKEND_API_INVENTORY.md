...
| POST | `/api/v1/admin/publications/{publication_id}/publish` | `backend/src/api/routers/admin_publications.py:545` | `_get_repo`, `_get_audit`, `_get_staleness_service` (Phase 3.1d) | `PublicationPublishRequest \| None` (`backend/src/schemas/staleness.py:173`) — optional body added in Phase 3.1d for `bound_blocks` snapshot capture; backward compatible (no body / null / `{}` all work; bare-array body → 422). **Phase 3.1d Slice 4b (Recon Delta 03)**: optional request header `If-Match: <strong-etag>` for optimistic concurrency. | 200 → `PublicationResponse` with response header `ETag: <new-strong-etag>`; if `If-Match` was absent the response also carries `Deprecation: true` (DEBT-079 v1 tolerance window). 404 → `PublicationNotFoundError`. **412 → `PublicationPreconditionFailedError`** when supplied `If-Match` does not match the row's current ETag; envelope shape identical to PATCH 412. | Phase 3.1d — when `bound_blocks` is present, invokes `PublicationStalenessService.capture_for_publication` with default `allow_auto_prime=True` (recon §5.3). Capture is best-effort: per-block resolve / shape-validation failures are logged inside the service and never raise into the caller, so publish success is never rolled back. Slice 4b adds `If-Match` precondition mirroring PATCH (Phase 1.3 §K); when the header is absent the handler proceeds (warn-log + `Deprecation: true`) per DEBT-079 — flips to required (428) after a two clean-week rollout window. |
| POST | `/api/v1/admin/publications/{publication_id}/compare` | `backend/src/api/routers/admin_publications.py:622` | `_get_staleness_service` (`PublicationStalenessService`) — composed via `_get_resolve_service` for cached-only resolve | n/a | 200 → `PublicationComparatorResponse` (`backend/src/schemas/staleness.py:148`); 404 → `PUBLICATION_NOT_FOUND` envelope | Phase 3.1d — read-only staleness comparator. Invariant `ARCH-CACHED-ONLY-RESOLVE-001`: invokes `ResolveService.resolve_value` with `allow_auto_prime=False` so the endpoint is strictly side-effect-free (no writes to `publication_block_snapshot`, no auto-prime, no audit rows). Publication with zero snapshot rows returns a synthetic `BlockComparatorResult{stale_reasons=["snapshot_missing"]}` with `overall_status="unknown"` (recon §3.4 BLOCKER-2 Option C). |
| GET | `/api/v1/admin/resolve/{cube_id}/{semantic_key}` | `backend/src/api/routers/admin_resolve.py` | `_get_resolve_service` (`ResolveService`) — wraps `SemanticMappingRepository`, `StatCanValueCacheService`, `StatCanMetadataCacheService` | n/a (query params) | 200 → `ResolvedValueResponse` (11 fields incl. `value: str \| null`, `missing: bool`, `cache_status: "hit" \| "primed"`); 400 → `RESOLVE_INVALID_FILTERS` (details include `expected`, `provided`, `reason`); 401 (middleware); 404 → `MAPPING_NOT_FOUND` (mapping absent or `is_active=false`) OR `RESOLVE_CACHE_MISS` (details include derived `coord`, `period`, `prime_attempted`, `prime_error_code`) | Phase 3.1c — singular cache-first resolve. Filter encoding: repeated `?dim=<position_id>&member=<member_id>` pairs + optional `?period=<ref_period>`. Service derives coord internally via `derive_coord(...)` (BLOCKER-1 Option B); caller never supplies `coord`. 8-step state machine (recon §5.2): mapping → parse/validate → coord → cache read → auto-prime → re-query → hit-or-miss. F-fix-3 missing-observation contract: cache rows with `value=NULL` round-trip as JSON `value: null, missing: true`. |
