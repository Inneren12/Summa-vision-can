...
| 2026-05-09 | PR-09 Slice 6 | (no production code changes) | Phase 3.1d closeout: component-level integration test suite at `frontend-public/tests/components/editor/phase-3-1d-closeout-integration.test.tsx` covering happy publish flow (Slice 4b sequencing — etagRef refresh → compare auto-fire → MARK_PUBLISHED), 412 conflict (PreconditionFailedModal `body_publish` copy + Reload re-fetches publication and updates etagRef), Pre-3.1d CTA (empty `block_results` + bindable doc → republish via lifted publish flow), and 7-reason tooltip i18n coverage (parametrized `it.each` over all `StaleReason` enum values). Full Playwright e2e harness deferred to Phase 4 per DEBT-080. |
