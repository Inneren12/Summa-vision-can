...
| 2026-05-07 | PR-08 Slice 5 | §7 | `CompareBadge` gains optional `reasons` prop + `ReasonsTooltip` (hover/focus disclosure, ARIA `aria-describedby` + `role="tooltip"`). 10 new i18n keys added under `publication.compare.reasons.*` (7) and `publication.compare.tooltip.*` (3), EN+RU parity. `aggregateReasons` + `shouldShowRepublishCta` added to `lib/utils/compareSeverity.ts`. `TopBar` renders standalone "Republish to refresh" CTA when any block has `snapshot_missing` (reuses existing `refresh_required.*` keys from Slice 1b — wired in this PR). `usePublishAction` + `<PublishConfirmModal>` lifted from `ReviewPanel` to editor root so TopBar CTA and ReviewPanel transition share a single hook instance + modal. |
