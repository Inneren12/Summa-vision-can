...
void main() {
  group('ContentBrief schema drift detection', () {
    ...
    setUpAll(() {
      ...
      final schemaFile = File('../backend/schemas/publication_response.schema.json');
      ...
      backendSchema = jsonDecode(schemaFile.readAsStringSync())
    ...
    test('Dart ContentBrief contains all backend required fields', () {
      final backendFields = (backendSchema['properties'] as Map<String, dynamic>).keys.toSet();
      ...
      for (final backendField in backendFields) {
        expect(
          ...
          isTrue,
          reason: 'Backend field "$backendField" missing from Dart ContentBrief.toJson()',
        );
      }
    });
    ...
    test('virality_score is numeric in backend schema', () {
      ...
      final isNumeric = scoreField['type'] == 'number' ||
          scoreField['type'] == 'integer' ||
          (anyOf is List && anyOf.any(
            (t) => t is Map && (t['type'] == 'number' || t['type'] == 'integer'),
          ));
