    ...
    <div style={{ padding: "6px 12px", borderBottom: `1px solid ${TK.c.brd}`, display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
      <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
        ...
        <div style={{ display: "flex", gap: "1px", background: TK.c.bgSurf, borderRadius: "3px", padding: "1px", border: `1px solid ${TK.c.brd}`, marginLeft: "6px" }}>
          {(["template", "design"] as const).map(m => <button type="button" key={m} onClick={() => setMode(m)} style={{ padding: "2px 7px", fontSize: "8px", fontFamily: TK.font.data, textTransform: "uppercase", background: mode === m ? TK.c.bgAct : "transparent", color: mode === m ? TK.c.acc : TK.c.txtM, border: "none", borderRadius: "2px", cursor: "pointer" }}>{m}</button>)}
        </div>
        <div style={{ display: "flex", gap: "2px", marginLeft: "8px" }}>
          <button type="button" onClick={() => dispatch({ type: "UNDO" })} disabled={!undoStack.length} style={{ padding: "2px 6px", fontSize: "10px", background: "transparent", border: "none", color: undoStack.length ? TK.c.txtS : TK.c.txtM, cursor: undoStack.length ? "pointer" : "default", opacity: undoStack.length ? 1 : .3 }} title="Undo">{"\u21A9"}</button>
          <button type="button" onClick={() => dispatch({ type: "REDO" })} disabled={!redoStack.length} style={{ padding: "2px 6px", fontSize: "10px", background: "transparent", border: "none", color: redoStack.length ? TK.c.txtS : TK.c.txtM, cursor: redoStack.length ? "pointer" : "default", opacity: redoStack.length ? 1 : .3 }} title="Redo">{"\u21AA"}</button>
        </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "5px" }}>
        ...
        <button type="button" onClick={() => fileRef.current?.click()} style={{ padding: "3px 6px", fontSize: "8px", fontFamily: TK.font.data, background: TK.c.bgSurf, color: TK.c.txtS, border: `1px solid ${TK.c.brd}`, borderRadius: "2px", cursor: "pointer" }}>IMPORT</button>
        <button type="button" onClick={exportJSON} style={{ padding: "3px 6px", fontSize: "8px", fontFamily: TK.font.data, background: TK.c.bgSurf, color: TK.c.txtS, border: `1px solid ${TK.c.brd}`, borderRadius: "2px", cursor: "pointer" }}>JSON</button>
        <button type="button" onClick={markSaved} disabled={!dirty} style={{ padding: "3px 6px", fontSize: "8px", fontFamily: TK.font.data, background: dirty ? TK.c.pos : TK.c.bgSurf, color: dirty ? TK.c.bgApp : TK.c.txtM, border: `1px solid ${dirty ? TK.c.pos : TK.c.brd}`, borderRadius: "2px", cursor: dirty ? "pointer" : "default", fontWeight: dirty ? 700 : 400, opacity: dirty ? 1 : .5 }} title="Ctrl+S">SAVE</button>
        <button
          type="button"
          onClick={exportPNG}
          disabled={!canExp}
