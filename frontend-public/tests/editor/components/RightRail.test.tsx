...
describe("RightRail", () => {
  ...
  test("clicking Review switches aria-selected and visible tabpanel", () => {
    ...
    // Review panel content should now be present
  ...
  test("Review tab pill shows total unresolved count across threads", () => {
    ...
    // One root with one reply (both unresolved), one resolved root (excluded).
    ...
    mountRail(state);
    // c1 unresolved (root) + c2 unresolved (reply) + c3 unresolved (root) = 3
    // c4 (root, resolved with no replies) excluded entirely.
